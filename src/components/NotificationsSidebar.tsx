...
export default function NotificationsSidebar({ open, onClose }: Props) {
    ...
    const FILTERS = [
        ...
        { key: 'renewal', label: t('notif_renewals') },
        { key: 'price_hike', label: t('notif_price_hikes') },
        { key: 'trial', label: t('notif_trials') },
    ];
    ...
    const panelStyle = isRTL
        ? { position: 'fixed' as const, top: 12, bottom: 12, left: 12, width: 380, zIndex: 9999 }
        : { position: 'fixed' as const, top: 12, bottom: 12, right: 12, width: 380, zIndex: 9999 };
        ...
        <>
            ...
            <div
                style={{
                    ...
                    animation: isRTL ? 'slideInLeft 0.3s cubic-bezier(0.16, 1, 0.3, 1)' : 'slideInRight 0.3s cubic-bezier(0.16, 1, 0.3, 1)',
            ...
            >
                ...
                <div style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--color-border)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <h2 style={{ fontSize: 18, fontWeight: 800 }}>{t('notif_title')}</h2>
                        {notifications.filter(n => !n.is_read).length > 0 && (
                            <span style={{ background: 'var(--color-accent-light)', color: 'var(--color-accent)', padding: '2px 8px', borderRadius: 12, fontSize: 11, fontWeight: 700 }}>
                                {notifications.filter(n => !n.is_read).length} {t('notif_new')}
                            </span>
                        )}
                    </div>
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
                    {loading ? (
                        <div style={{ padding: 20, textAlign: 'center', color: 'var(--color-text-tertiary)' }}>{t('notif_loading')}</div>
                            ...
                            {filteredNotifications.map(n => {
                                ...
                                return (
                                    <div key={n.id} style={{
                                        ...
                                        {!n.is_read && <div style={{ position: 'absolute', top: 16, [isRTL ? 'left' : 'right']: 16, width: 6, height: 6, borderRadius: '50%', background: 'var(--color-accent)' }} />}
                                        ...
                                        <div style={{ flex: 1 }}>
                                            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>{t('notif_item_title')}</div>
                                            ...
                                            {n.type === 'renewal_reminder' && (
                                                <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                                                    <button className="btn btn-secondary btn-sm" style={{ padding: '4px 12px', fontSize: 12 }}>{t('notif_review')}</button>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                ...
                <div style={{ padding: '16px 24px', borderTop: '1px solid var(--color-border)', textAlign: 'center' }}>
                    <button className="btn btn-ghost" style={{ width: '100%', justifyContent: 'center', fontSize: 14, color: 'var(--color-text-secondary)' }}>
                        {t('notif_view_all')} <ArrowUpRight size={16} style={{ marginInlineStart: 4 }} />
                    </button>
                </div>
            </div>
